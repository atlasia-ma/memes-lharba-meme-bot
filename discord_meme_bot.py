...
async def get_meme(ctx, *, text=None):
    try:
        ...
        # Handle Gradio tuple response
        if result and isinstance(result, (list, tuple)) and len(result) >= 2:
            html_content, description = result[0], result[1]

            # Extract URL from iframe in the HTML content
            match = re.search(r'src="([^"]+)"', str(html_content))
            if match:
                meme_url = match.group(1)

                # Clean up Google Drive URL for better embedding
                if 'drive.google.com' in meme_url and '/preview' in meme_url:
                    meme_url = meme_url.replace('/preview', '')
        ...
        # Check if we have a valid result with a filename (fallback)
